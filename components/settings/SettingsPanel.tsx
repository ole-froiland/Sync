...
import { AlertCircle, CheckCircle, GitBranch, X } from 'lucide-react'
...
export default function SettingsPanel({ className, historyPath = '/settings' }: SettingsPanelProps) {
  ...
  // Read URL params once on mount to set the initial flash.
  ...
  // Side effects only: clean URL + refresh context.
  useEffect(() => {
    const params = new URLSearchParams(window.location.search)
    if (historyPath && (params.get('github_connected') || params.get('github_error'))) {
      ...
      {flash && (
        <div
          className={`mb-5 flex items-center gap-3 rounded-xl px-4 py-3 text-sm ${
            flash.type === 'success'
              ? 'border border-emerald-100 bg-emerald-50 text-emerald-700 dark:border-emerald-900/50 dark:bg-emerald-950/40 dark:text-emerald-400'
              : 'border border-red-100 bg-red-50 text-red-700 dark:border-red-900/50 dark:bg-red-950/40 dark:text-red-400'
          }`}
        >
          {flash.type === 'success' ? (
          ...
          <button onClick={() => setFlash(null)} className="opacity-60 transition-opacity hover:opacity-100">
            <X size={14} />
          </button>
        </div>
      )}
        ...
        <Card>
          ...
          <div className="flex items-center justify-between border-b border-gray-100 py-3 dark:border-gray-800">
            <div className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 bg-gray-900 dark:border-gray-700 dark:bg-gray-100">
            ...
            <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-medium text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-400">
              Connected
            </span>
          </div>
